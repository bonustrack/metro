...
1. **Local mode.** Agent directory, `fileSource`, admin socket, `init`/`add`/`remove`/`tools`/`status`/`logs`/`tail`/`claude`/`doctor`, ids gone. Interim key: derived from a pasted signature (no WalletConnect yet), so only one key scheme ever exists. *Revised 2026-09-04: the UI stays hosted on metro.box and talks to the local daemon (runtime daemon address, `/api/mode`), so stations and connectors are added from the UI rather than the CLI; sign-in stays the wallet, bound to the owner in the agent file. First slice shipped: `fileSource` behind `METRO_MODE=local`, the daemon boots from `~/.metro/agents/*/agent.json` with no database. Second slice shipped: the local daemon's own side — owner claim on first wallet sign-in, a minted session secret, `/api/mode`, and file-backed agent, key and station routes. Third slice shipped: the hosted UI talks to a chosen daemon (`#/connect`, the link a local daemon prints, mode-aware pages).*
