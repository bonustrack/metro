/**
 * Unit tests for `trainEventToMetroEvent` (src/routes/http.ts) — the
...
describe('trainEventToMetroEvent — required line', () => {
  test('drops event without a `line` (returns null)', () => {
    expect(trainEventToMetroEvent({} as TrainEvent, 'tg')).toBeNull();
  });

  test('drops event with non-string `line`', () => {
    expect(trainEventToMetroEvent({ line: 123 as unknown as string }, 'tg')).toBeNull();
  });
});
...
describe('trainEventToMetroEvent — defaults for omitted fields', () => {
  ...
  test('station: falls back to trainName when line has no parseable station', () => {
    ...
    const e = trainEventToMetroEvent({ line: 'not-a-line' }, 'my-train');
...
describe('trainEventToMetroEvent — passthrough fields', () => {
