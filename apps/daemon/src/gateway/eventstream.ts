...
function readHeaderValue(buf: Buffer, at: number, type: number): { value: string; next: number } {
  if (type === HEADER_BYTES || type === HEADER_STRING) {
    const len = buf.readUInt16BE(at);
    const raw = buf.subarray(at + 2, at + 2 + len);
    const value = type === HEADER_STRING ? raw.toString('utf8') : raw.toString('base64');
...
export function decodeFrame(buf: Buffer, offset: number): { message: EventStreamMessage; end: number } | null {
  ...
  if (total < PRELUDE_BYTES + CRC_BYTES || headersLen > total) throw new Error('eventstream: malformed frame prelude');
...
export function encodeFrame(headers: Record<string, string>, payload: Buffer): Buffer {
  const headerBuf = Buffer.concat(Object.entries(headers).map(([name, value]) => encodeHeader(name, value)));
