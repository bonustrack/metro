...
import type { Pending } from '@metro-labs/core/trains/protocol';
...
export function startWatcher(
  ...
  try {
    const w = watch(dir, (_event, filename) => {
      if (!filename) return;
      const base = filename;
      if (!TRAIN_EXT.test(base) || base.startsWith('_') || base.startsWith('.'))
        return;
    ...
    w.on('error', (err) => {
      log.warn({ err: errMsg(err) }, 'train hot-reload: watcher error');
    });
    ...
    log.warn(
      { err: errMsg(err) },
      'train hot-reload: watch unavailable (on-demand restart only)',
    );
    return null;
  }
}
