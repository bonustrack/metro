...
import {
  ...
  parseTrainLine,
  sendCall,
  type TrainCallResponse,
  type TrainEvent,
} from '@metro-labs/core/trains/protocol';
import {
...
import { trainsDir } from '../boot/paths.js';
...
export class TrainSupervisor {
  ...
  start(): void {
    ...
    log.info(
      { dir: this.dir, count: this.trains.size },
      'train supervisor: started',
    );
  ...
  async call(
    ...
    if (!t)
      throw new Error(
        `no train named '${name}' (have: ${[...this.trains.keys()].join(', ') || '(none)'})`,
      );
  ...
  private pumpStdout(state: TrainState): Promise<void> {
    return pumpStream(state.proc?.stdout, state.name, 'stdout', (chunk) => {
      state.buf += chunk;
      state.buf = drainLines(state.name, state.buf, (line) => {
  ...
  private onExit(state: TrainState, code: number): void {
    ...
    failAllPending(
      state.pending,
      `train '${state.name}' exited (code=${code}) before responding`,
    );
    if (state.stopped) return;
    state.failCount++;
    if (state.failCount >= MAX_CONSECUTIVE_FAILS) {
      log.error(
        { name: state.name, fails: state.failCount },
  ...
  private scheduleRestart(state: TrainState): void {
    ...
    log.info(
      { name: state.name, delay, attempt: state.failCount },
      'train: restart scheduled',
    );
    state.restartTimer = setTimeout(() => {
      state.restartTimer = null;
      this.spawn(state);
      setTimeout(() => {
        if (state.proc?.exitCode === null) state.failCount = 0;
      }, 30_000);
    }, delay);
  }
}
