...
describe('the signed-in account on the page', () => {
  ...
  test('the exchange answer becomes an account, with the organization and role from the token', () => {
    ...
    expect(account).toEqual({ accessToken: token, refreshToken: 'rt', organization: 'org_1', organizationName: null, organizationSlug: null, role: 'admin', user: { id: 'user_1', email: 'admin@stage.box', name: 'Stage Labs', picture: null } });
