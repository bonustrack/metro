...
function toMember(value: unknown): Member | null {
  ...
  return { membershipId: value.membershipId, userId: value.userId, email: text(value.email), name: text(value.name), picture: text(value.picture), role: roleOf(value.role) };
}
...
function toInvitation(value: unknown): Invitation | null {
  ...
  return { id: value.id, email: value.email, role: value.role === undefined || value.role === null ? null : roleOf(value.role), expiresAt: text(value.expiresAt) };
}
