...
export interface AgentBundle {
  version: 1;
  agent: { id: string; name: string; stations: { station: string }[] };
  connectors: unknown[];
}
...
export async function fetchBundle(agentId: string): Promise<AgentBundle> {
  ...
  if (typeof agent.id !== 'string') throw unexpected();
  ...
  return {
    version: 1,
    agent: { ...agent, id: agent.id, name: typeof agent.name === 'string' ? agent.name : '', stations },
