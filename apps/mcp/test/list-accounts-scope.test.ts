...
describe('scopeAccountsByAgent', () => {
  ...
  test('agentIdForAccount resolves the owning agent id', () => {
    ...
    expect(agentIdForAccount('telegram-bot', 't0')).toBe('agent000002');
  ...
  test('agentForLine still resolves the display name through the id', () => {
    ...
    expect(agentForLine('metro://telegram-bot/t0/chat1')).toBe('wan');
  ...
  test('keeps only accounts owned by an allowed agent', () => {
    ...
    expect(scoped['discord-bot']).toEqual([{ id: 'd0' }]);
    expect(scoped['telegram-bot']).toEqual([]);
  });
...
describe('two agents with the same name, different owners', () => {
  beforeEach(() =>
    setAgentMap(
      { 'telegram-bot/ada-tg': 'agent000007', 'telegram-bot/bob-tg': 'agent000008' },
  ...
  test('each id sees only its own account', () => {
    const byStation = { 'telegram-bot': [{ id: 'ada-tg' }, { id: 'bob-tg' }] };
    expect(scopeAccountsByAgent(byStation, new Set(['agent000007']))['telegram-bot']).toEqual([
      { id: 'ada-tg' },
    ]);
    expect(scopeAccountsByAgent(byStation, new Set(['agent000008']))['telegram-bot']).toEqual([
      { id: 'bob-tg' },
    ]);
  });

  test('the shared name is not a key — both accounts still report it', () => {
    expect(agentForLine('metro://telegram-bot/ada-tg/c1')).toBe('tony');
    expect(agentForLine('metro://telegram-bot/bob-tg/c1')).toBe('tony');
    expect(agentIdForAccount('telegram-bot', 'ada-tg')).toBe('agent000007');
    expect(agentIdForAccount('telegram-bot', 'bob-tg')).toBe('agent000008');
  });

  test('two accounts of the same station carry their own agent id', () => {
    const tagged = attachAgentIds({ 'telegram-bot': [{ id: 'ada-tg' }, { id: 'bob-tg' }] });
    expect(tagged['telegram-bot']).toEqual([
