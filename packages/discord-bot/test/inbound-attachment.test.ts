 ...
 *    answered 401 for discord-bot files while answering 200 for every other
 *    station's. With no url on the payload the daemon mints the grant, exactly
 *    as it does for telegram-bot, telegram and xmtp.
 *
...
beforeAll(() => {
  process.env.METRO_XMTP_ATTACH_DIR = mkdtempSync(
    join(tmpdir(), 'metro-discord-bot-att-'),
  );
  globalThis.fetch = (() =>
...
describe('discord-bot attachmentSaved payload', () => {
...
describe('discord-bot voice notes', () => {
