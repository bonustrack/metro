...
describe('discord-bot groupCreate', () => {
  test('creates a thread and adds members (one blocked)', async () => {
    ...
    expect(res.line).toBe('metro://discord-bot/acc/th9');
...
describe('discord-bot groupAddMembers', () => {
  test('PUTs each member onto the thread', async () => {
    const calls: Call[] = [];
    const rest = fakeRest({ calls });
    const res = await groupAddMembers(rest, 'acc', 'th1', 'metro://discord-bot/acc/th1', {
      members: ['1', '2'],
    });
    expect(res.members).toEqual([
      { id: '1', status: 'added' },
...
describe('discord-bot groupRemoveMembers', () => {
  test('DELETEs each member from the thread', async () => {
    const calls: Call[] = [];
    const rest = fakeRest({ calls });
    const res = await groupRemoveMembers(rest, 'acc', 'th1', 'metro://discord-bot/acc/th1', {
      members: ['1'],
    });
