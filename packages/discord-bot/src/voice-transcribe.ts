...
function emitTranscript(
  ...
  emitInbound(s.accountId, {
    ...
    station: 'discord-bot',
    line: `metro://discord-bot/${s.accountId}/voice/${s.channelId}`,
    line_name: 'voice',
    from: `metro://discord-bot/${s.accountId}/user/${userId}`,
...
async function handleSpeaker(s: Session, userId: string): Promise<void> {
  ...
  try {
    ...
    process.stderr.write(
      `discord-bot voice transcribe error (${userId}): ${errMsg(err)}\n`,
    );
  } finally {
...
export function startTranscription(
  ...
  s.onStart = (userId: string) => {
    handleSpeaker(s, userId).catch((err: unknown) => {
      process.stderr.write(
        `discord-bot[${accountId}] voice speaker failed: ${errMsg(err)}\n`,
      );
    });
  };
  ...
  process.stderr.write(
    `discord-bot[${accountId}] voice transcription armed on ${channelId}\n`,
  );
}
