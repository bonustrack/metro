...
import { readJson, writeJson } from '@metro-labs/metro/secure-fs';
...
async function fcmAccessToken(): Promise<string | null> {
  ...
  const grant = encodeURIComponent(
  ...
  if (!res.ok) {
    process.stderr.write(
      `fcm token exchange ${res.status}: ${await res.text()}\n`,
    );
    return null;
  }
...
async function fcmPushTo(
  ...
  if (!res.ok) {
    ...
    if (dead) {
      ...
      process.stderr.write(
        `fcm: pruned stale token ${deviceToken.slice(0, 12)}…\n`,
      );
      return;
    }
...
export async function fcmPushToAll(
  ...
  const tokens = loadPushTokens().filter((t) => {
    ...
    if (
      excludeInboxId &&
      (t.inboxId === excludeInboxId || t.inboxIds?.includes(excludeInboxId))
    )
      return false;
    return true;
  });
  ...
  await Promise.all(
    deduped.map((t) =>
      fcmPushTo(t.token, { ...data, account: accountId }).catch(
        () => undefined,
      ),
    ),
  );
