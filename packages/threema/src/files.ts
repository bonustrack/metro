...
import { emitInbound, reportAttachment, type SavedFile } from '@metro-labs/core/stations/train-events';
...
async function fetchFile(acct: Account, m: InboundMeta, file: FileData): Promise<SavedFile> {
  ...
  return saveBufferToCache(plain, m.messageId, 0, { mime: file.mime, name: file.name });
}
...
export function deliverFile(acct: Account, m: InboundMeta, file: FileData, room: Room, sentByUs: (id: string) => boolean): void {
  ...
  reportAttachment(fetchFile(acct, m, file), { station: 'threema', account: acct.cfg.id, line, forId, index: 0 }, { saved: { kind }, failed: { kind } });
}
