...
beforeEach(() => {
  ...
  process.env.TELEGRAM_ACCOUNTS_FILE = join(dir, 'missing.json');
});
...
describe('telegram account store', () => {
...
describe('telegram line helpers', () => {
  test('lineOf builds account-scoped lines with optional topic', async () => {
    ...
    expect(lineOf('acct', -100, 7)).toBe('metro://telegram/acct/-100/7');
    expect(lineOf('acct', -100)).toBe('metro://telegram/acct/-100');
  });
  ...
  test('targetOf rejects non-telegram and malformed lines', async () => {
    ...
    expect(targetOf('metro://telegram/acct/not-a-number')).toBeUndefined();
  });
});
