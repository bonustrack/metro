...
describe('telegram groupCreate', () => {
  test('surfaces missing invitees as invited with an invite link', async () => {
    ...
    expect(res.line).toBe('metro://telegram/default/900');
...
describe('telegram groupRemoveMembers', () => {
  test('kicks each member and reports removed', async () => {
    const c = calls();
    const client = fakeClient({ calls: c });
    const res = await groupRemoveMembers(client, 900, 'metro://telegram/default/900', {
      members: ['5', '7'],
    });
...
describe('telegram groupInviteLink', () => {
  test('exports the chat invite link', async () => {
    const c = calls();
    const client = fakeClient({ link: 'https://t.me/+xyz', calls: c });
    const res = await groupInviteLink(client, 900, 'metro://telegram/default/900');
