...
const LINE = 'metro://telegram/default/12345';
...
describe('telegram outbound handlers', () => {
  let calls: Captured[];
  ...
  test('send to a forum-topic line routes into the topic via replyTo', async () => {
    ...
    await handle({
      ...
      id: 't',
      action: 'send',
      args: { line: 'metro://telegram/default/-1004345680824/2', text: 'hi topic' },
    });
    cap.restore();
    expect(calls[1]?.method).toBe('sendText');
  ...
  test('explicit replyTo wins over the topic id from the line', async () => {
    ...
    await handle({
      ...
      id: 't2',
      action: 'reply',
      args: { line: 'metro://telegram/default/-1004345680824/2', text: 'yo', replyTo: '55' },
    });
    cap.restore();
    expect(calls[1]?.method).toBe('sendText');
  ...
  test('read calls getHistory and returns the xmtp-shaped result', async () => {
    ...
    expect(cap.responses[0]).toMatchObject({
      ...
      result: {
        ...
        count: 2,
        messages: [
          { id: '8', from: 'metro://telegram/default/user/111', text: 'older' },
          { id: '9', from: 'metro://telegram/default/user/222', text: 'newest' },
        ],
      },
    });
  });
