...
export class TelegramLogin {
  ...
  constructor(private creds: TelegramCredentials) {
  ...
  async requestCode(): Promise<{ step: TelegramStep; length: number }> {
    await this.tg.connect();
    const sent = await this.tg
      .sendCode({ phone: this.creds.phone })
      .catch((err: unknown) => {
        throw new TelegramLoginError(readableAuthError(err));
      });
    if (!isSentCode(sent))
      throw new TelegramLoginError(
  ...
  async submitCode(raw: string): Promise<TelegramSignedIn | 'password'> {
    const phoneCode = raw.replace(/[^0-9]/g, '');
    if (phoneCode === '')
      throw new TelegramLoginError('enter the login code Telegram sent you');
    try {
  ...
  async submitPassword(password: string): Promise<TelegramSignedIn> {
    if (password === '')
      throw new TelegramLoginError(
  ...
  private async exportFor(user: UserLike): Promise<TelegramSignedIn> {
