...
async function postForm(path: string, fields: Record<string, string>, fetchImpl: FetchLike): Promise<{ status: number; body: Body }> {
  let res: Response;
  try {
...
export function failureOf(body: Body, id = clientId()): string {
  ...
  if (needsConsent(error, codes))
    return `An administrator of this Microsoft 365 organization must approve Metro once before it can read this mailbox. They can do it at https://login.microsoftonline.com/common/adminconsent?client_id=${id} and then you can start again.`;
...
function tokensOf(body: Body, now: number, previous?: string): Tokens {
