...
1. If `$ARGUMENTS` is empty, do not run anything. Tell the user to open https://metro.box/#/authorize, pick the agent (or press Authorize a machine on the agent's own page), copy the pairing code (`ma_…`), and run `/metro:login <code>`. Stop.
