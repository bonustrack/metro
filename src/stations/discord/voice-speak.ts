...
export async function speak(
  ...
  if (!text) {
    respond(id, { result: { ok: false, error: 'text is required' } });
    return;
  }
  ...
  if (!client.isReady()) {
    respond(id, {
      result: { ok: false, error: `gateway not ready for '${accountId}'` },
    });
    return;
  }
  ...
  if ('error' in resolved) {
    respond(id, { result: { ok: false, error: resolved.error } });
    return;
  }
  ...
  try {
    ...
    respond(id, {
      result: { ok: true, spoke: text, channelName, account: accountId },
    });
  } catch (err) {
    respond(id, {
