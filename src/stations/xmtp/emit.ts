...
function multiRemotePayload(
  ...
  const text =
    imgCount === attachments.length && imgCount > 1
      ? `📷 ${imgCount} photos`
      : vidCount === attachments.length && vidCount > 1
        ? `🎥 ${vidCount} videos`
        : attachments.length === 1 && one
          ? one.kind === 'video'
            ? `🎥 ${one.name ?? 'video'}`
            : `[${one.kind}: ${one.name ?? one.url}]`
...
export function envelope(
  ...
  if (typeId === 'reaction' && c && typeof c === 'object')
    return reactionPayload(base, c as Reaction);
  if (typeId === 'reply' && c && typeof c === 'object') {
    ...
    return {
      ...
      payload: {
        contentType: typeId,
        replyTo: r.reference,
        replyContentType: r.contentType?.typeId,
      },
    };
  }
  if (typeId === 'attachment' && c && typeof c === 'object') {
    ...
    const out = {
      ...
      payload: {
        contentType: typeId,
        attachments: [{ kind, mime: a.mimeType, name: a.filename, dataB64 }],
      },
    };
    if (kind === 'audio')
      void transcribeAndEmit(a.content, line, accountId, base.id, emitInbound);
    emitAttachmentSaved(
      accountId,
      line,
      base.id,
      0,
      saveInlineAttachment(a, msg.id, 0),
    );
    return out;
  }
  if (typeId === 'remoteStaticAttachment' && c && typeof c === 'object') {
    ...
    emitAttachmentSaved(
      ...
      base.id,
      0,
      saveRemoteAttachment(r, msg.id, 0),
    );
    return {
      ...base,
  ...
  if (
    ...
    (Array.isArray(m.attachments) ? m.attachments : []).forEach((r, i) => {
      emitAttachmentSaved(
        ...
        base.id,
        i,
        saveRemoteAttachment(r, msg.id, i),
      );
    });
  ...
  if (typeId === 'walletSendCalls' && c && typeof c === 'object') {
    return {
      ...
      payload: {
        contentType: typeId,
        walletSendCalls: c as WalletSendCallsParams,
      },
    };
  }
  if (typeId === 'transactionReference' && c && typeof c === 'object') {
    return {
      ...
      payload: {
        contentType: typeId,
        transactionReference: c as TransactionReference,
      },
    };
  }
  if (typeId === 'signatureRequest' && c && typeof c === 'object') {
    return {
      ...
      payload: { contentType: typeId, signatureRequest: c },
    };
  }
  if (typeId === 'signatureReference' && c && typeof c === 'object') {
    return {
      ...base,
      text: 'Signature',
      payload: { contentType: typeId, signatureReference: c },
    };
  }
  return {
    ...base,
