...
| `GET /api/model/codex/models` | What the ChatGPT account may use. OpenAI filters this by the client version metro announces (`METRO_CODEX_VERSION`, 0.153.4 by default), so a model newer than that version is withheld from the list even when the account has it. |
| `GET /api/model/openrouter/models` | What OpenRouter serves, proxied from its public list so the Model tab can offer a searchable picker instead of a typed model id. |
| `GET`/`PUT /api/model` | Where Claude Code's inference goes for sessions started with `metro claude` on this box: Anthropic (your Claude Code login passed through, or an API key kept here, with an optional model pin that never upgrades a Haiku request), Amazon Bedrock, OpenRouter, or Codex on your ChatGPT subscription (sign in from the page; unofficial, it speaks the Codex CLI protocol), with the provider keys and tokens kept on the daemon and never returned. The **Model** tab edits it; the change applies to the next request, no restart. |
...
You need an IAM user with `ec2:DescribeRegions`, `ec2:DescribeImages`, `ec2:DescribeInstances`,
`ec2:RunInstances` and `ec2:CreateTags`, and a single-use Tailscale auth key. The box joins your tailnet as
